...
/*connection to post comments*/
	...
	if(isset($_POST['comment_text'])){$sql = "INSERT INTO food_bar_comments (id,user_id, user_name, body, created_date) VALUES ('','$user_id_for_comments','$sesson_user_name','$comment_text', now())";
	if(mysqli_query($comments_db, $sql)){}else{echo "Error: Cannot connect to the table";}}

/*get the no of comments*/
	$sql_get_last_id = "SELECT MAX(id) as rum From food_bar_comments";
	...
	$sql_get_last_id_fetch = mysqli_fetch_array($sql_get_last_id_query);
     ...
     /*loop to show all the comments on the food and bar page*/	
     	 for ($i=1; $i<=$max_id ; $i++) { 

				$get_comment = "SELECT body,user_name,created_date FROM food_bar_comments Where id=".$i;
