...
<link rel="stylesheet" type="text/css" href="styles/salo-iot-campus-guide-style.css"/>
...
?>
               ...
               <!-- <iframe class="salo-iot-map" src="https://www.google.com/maps/d/u/0/embed?mid=1VTCKswkrdAtcbI1LzxH0Mv-eUBrwvV61">
                </iframe> -->
                
                <img class='salo_campus_polarised' id="polarised1_1"  src="<?php
  
  $heading = $connect_database_crud->query("SELECT source FROM Images WHERE comments = 'Salo IoT Campus'") or die($connect_database_crud->error);
  while($row = $heading->fetch_assoc()){
    echo $row['source'];
  }
...
?>" alt="Salo IoT Campus">
			<div class='polarisedtext' id="polarisedtext0_1" onclick="checkSalo()">Show Map</div>
                </div>
                </aside>
                <div class="paragraph-salo-iot-campus">
                
                <p>

                    <?php
  
  $heading = $connect_database_crud->query("SELECT information FROM travel_guide_data_part1_data WHERE comments = 'Para1'") or die($connect_database_crud->error);
  while($row = $heading->fetch_assoc()){
    echo $row['information'];
  }
...
?>
  ...
  $heading = $connect_database_crud->query("SELECT information FROM travel_guide_data_part1_data WHERE comments = 'Para2'") or die($connect_database_crud->error);
  while($row = $heading->fetch_assoc()){
    echo $row['information'];
  }
...
?> </p>
  ...
  $heading = $connect_database_crud->query("SELECT information FROM travel_guide_data_part1_data WHERE comments = 'Para3'") or die($connect_database_crud->error);
  while($row = $heading->fetch_assoc()){
    echo $row['information'];
  }
...
?>
  ...
  $heading = $connect_database_crud->query("SELECT information FROM travel_guide_data_part1_data WHERE comments = 'Para4'") or die($connect_database_crud->error);
  while($row = $heading->fetch_assoc()){
    echo $row['information'];
  }
...
?>
  ...
  $heading = $connect_database_crud->query("SELECT information FROM travel_guide_data_part1_data WHERE comments = 'Para5'") or die($connect_database_crud->error);
  while($row = $heading->fetch_assoc()){
    echo $row['information'];
  }
...
<link rel="stylesheet" href="Styles/admin-panel.css">
    ...
    ?>
    <script src="scripts/salo-iot-campus-guide-script.js"> </script>
    <style>
  .heading-panel{
    float:right;
    color:white;
