...
<link rel="stylesheet" type="text/css" href="styles/bus-travel-guide-style.css">
<body>
<header>
  <?php
...
<div id="main-content">
  ...
  $heading = $connect_database_crud->query("SELECT information FROM travel_guide_data_part2_data WHERE comments = 'main Heading'") or die($connect_database_crud->error);
...
?>
                ...
                <div class="pol" id="pol1_1">
                                <img class="bus_station_polarised" id="polarised1_1" src="<?php
  
  $heading = $connect_database_crud->query("SELECT source FROM Images WHERE comments = 'Salo Bus Station'") or die($connect_database_crud->error);
  while($row = $heading->fetch_assoc()){
    echo $row['source'];
  }


?>" alt="Salo Bus Station" /> 

                <div class="polarisedtext" id="polarisedtext0_1" onclick="checkBus()">Show Map</div>
                </div>
                </aside>
                <div class="paragraph-bus-travel-guide">
                
                <p>
                    <?php
  
  $heading = $connect_database_crud->query("SELECT information FROM travel_guide_data_part2_data WHERE comments = 'para1'") or die($connect_database_crud->error);
  while($row = $heading->fetch_assoc()){
    echo $row['information'];
  }
...
?>
  ...
  $heading = $connect_database_crud->query("SELECT information FROM travel_guide_data_part2_data WHERE comments = 'para2'") or die($connect_database_crud->error);
  while($row = $heading->fetch_assoc()){
    echo $row['information'];
  }
...
?>
  ...
  $heading = $connect_database_crud->query("SELECT information FROM travel_guide_data_part2_data WHERE comments = 'para3'") or die($connect_database_crud->error);
  while($row = $heading->fetch_assoc()){
    echo $row['information'];
  }
...
<link rel="stylesheet" href="Styles/admin-panel.css">
        <script src="scripts/bus-travel-guide-script.js"></script>
